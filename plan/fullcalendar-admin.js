class CalendarAdmin {
    ...
    handleDateClick(info) {
        ...
        if (viewType === 'dayGridMonth') {
            // Месячный вид - создаем событие на весь день
            eventData.extendedProps = {
                eventType: 'allDay'
            };
        } else {
            // Недельный или дневной вид - создаем событие с временем
    ...
    handleSelect(selectionInfo) {
        ...
        if (viewType === 'dayGridMonth') {
            // Месячный вид - выделение диапазона дат
            ...
            // Недельный или дневной вид - выделение временного диапазона
            ...
            if (isTimeRange) {
                ...
                // Одиночный клик во временном виде
                eventData.extendedProps = {
                    startTime: this.formatTimeInput(start),
                    endTime: this.formatTimeInput(new Date(start.getTime() + 60 * 60 * 1000)),
                    eventType: 'timeRange'
                };
            }
        }
    ...
    openEventModal(eventData = null) {
        ...
        if (eventData && eventData.id) {
            ...
            // Устанавливаем выбранную дату/время из клика или выделения
            ...
            if (eventData && eventData.start) {
                if (eventDateField) eventDateField.value = this.formatDateInput(eventData.start);
                
                if (eventData.extendedProps) {
                    if (eventData.extendedProps.endDate && endDateField) {
                        endDateField.value = eventData.extendedProps.endDate;
                    }
                    if (eventData.extendedProps.startTime && startTimeField) {
                        startTimeField.value = eventData.extendedProps.startTime;
                    }
                    if (eventData.extendedProps.endTime && endTimeField) {
                        endTimeField.value = eventData.extendedProps.endTime;
                    }
                }
            } else {
                // Если нет данных события, устанавливаем текущую дату
                const now = new Date();
    ...
    setDefaultValues() {
        ...
        if (bigHallCheckbox && !bigHallCheckbox.checked) {
        ...
        if (romanovCheckbox && !romanovCheckbox.checked) romanovCheckbox.checked = true;
        if (toitonovCheckbox && !toitonovCheckbox.checked) toitonovCheckbox.checked = true;
    }
    ...
    fillFormWithEventData(eventData) {
        ...
        if (!eventData.id) {
            ...
            if (eventTitleField && !eventTitleField.value) eventTitleField.value = 'Сценическая репетиция + монтаж';
